...
function send_ajax(){
	//$('#tmpimg').remove();
	click_x = $('#clkx_ajax').val();
	click_y = $('#clky_ajax').val();
	win_w = $('#winw_ajax').val();
	win_h = $('#winh_ajax').val();
...
function clip_zip(){
	if (parent.data.document.forms.fm4){
		if(parent.functions.location.pathname.indexOf("single.php") != -1 ){
			...
			parent.data.document.forms.fm4.sppcode.value = sppcode;
		} else {
			parent.data.document.forms.fm4.sppcode.value = '';
...
<body >
<img id="loader_gif" alt="loading icon" style="position: absolute; top: 100px; left: 100px; display: block; z-index: 8;" src="/graphics/segap/ajax-loader.gif" />
...
<table>
<tr>
<td><input type="image" src="/graphics/segap/mag_plus_up.png" id="zmin" title="Zoom In" onclick="zoom_in()" /></td>
<td><input type="image" src="/graphics/segap/mag_minus_up.png" id="zmout" title="Zoom Out" onclick="zoom_out()" /></td>
<td><input type="image" src="/graphics/segap/pan_up.png" id="pn" title="Pan" onclick="pan()" /></td>
<td><input type="image" src="/graphics/segap/draw_up.png" style="visibility:hidden;"/> </td>
<td><input  type="image" src="/graphics/segap/info_up.png"  style="visibility:hidden;"/> </td>
<td><input  type="image" src="/graphics/segap/info_up.png"  style="visibility:hidden;"/> </td>
<td><input  type="image" src="/graphics/segap/info_up.png"  style="visibility:hidden;"/> </td>
<td><input  type="image" src="/graphics/segap/info_up.png"  style="visibility:hidden;"/> </td>
<td><input type="image" src="/graphics/segap/edit_user_up.png" title="login" onclick="login2();" /> </td>
<td >User: </td>
<td ><input type="text" size="20" id="visitor" value="<?php  echo $user_name; ?>" readonly='readonly' /></td>
</tr>

<tr>
<td><input type="image" src="/graphics/segap/se_full.png" title="Zoom to Full Extent" onclick="fullview()" /></td>
<td><input type="image" src="/graphics/segap/zoom_aoi_up.png" title="zoom to AOI" onclick="zoom_aoi()" /></td>
<td><input type="image" src="/graphics/segap/new_aoi_up.png" title="create different AOI" onclick="new_aoi();" /></td>
<td><input type="image" src="/graphics/segap/resize_up.png" title="reload frame size of map" onclick="resize();" /></td>
<td><input type="image" src="/graphics/segap/aoi_up.png" title="Use AOI" onclick="save_aoi();" /> </td>
<td><input type="image" src="/graphics/segap/pdf_up.png" title="export pdf" onclick="export_pdf();" /> </td>
<td><input type="image" src="/graphics/segap/data_dnld.png" title="data download as zip file" onclick="clip_zip()" /> </td>
<td><input type="image" src="/graphics/segap/help_up.png" title="Help" onclick="help();" /></td>
</tr>
</table>
</div>
...
if(isset($_GET['species'])){
	...
	//make these accept input from get for permalink
	$state_aoi = $_REQUEST['state'];
	$type = $_REQUEST['type'];
	$layer = $_REQUEST['layers'];
	$ecosys_aoi = $_REQUEST['ecosys'];
...
<input type="hidden" name="win_w" id="winh_pdf"  value="<?php  echo $win_w; ?>" />
<input type="hidden" name="win_h" id="winw_pdf" value="<?php  echo $win_h; ?>" />
<input type="hidden" name="dpi"  />
<input type="hidden" name="desc"  />
<input  type="hidden" name="species_layer" id="species_layer_pdf" />
<input  type="hidden" name="sppcode" id="sppcode_pdf" />
...
<input type="hidden" id="pred_transp_ajax" value="75" />
<input type="hidden" id="range_transp_ajax" value="25" />
...
<input type="hidden" class="aoikeys" id="lcc_ajax" value="<?php  echo $lcc_aoi; ?>" />
...
<input  type="hidden" id="sppcode_ajax" value="<?php echo $link_species; ?>" />
<input  type="hidden" id="species_layer_ajax" value="<?php echo $link_predicted; ?>" />
<input  type="hidden" id="species_layer_prev_ajax" />
